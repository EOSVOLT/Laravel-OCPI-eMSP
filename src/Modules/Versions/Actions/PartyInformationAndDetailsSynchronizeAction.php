...
class PartyInformationAndDetailsSynchronizeAction
{
    public function handle(Party $party): Party
    {
        ...
        Log::channel('ocpi')->info('['.$party->code.'] OCPI GET call for Versions Information of the Party on '.$party->url);
        $ocpiClient = new OcpiClient($party, 'versions.information');
        $versionList = $ocpiClient->versions()->information();
        throw_if(
            ! is_array($versionList),
            new Exception('['.$party->code.'] Empty or invalid response for Versions Information.')
        );
        ...
        throw_if(
            ...
            new Exception('['.$party->code.'] No version found.')
        );

        Log::channel('ocpi')->info('['.$party->code.'] Set Party OCPI version to '.$currentItem['version']);
        $party->version = $currentItem['version'];
        $party->version_url = $currentItem['url'];
        throw_if(
            ! $party->save(),
            new Exception('['.$party->code.'] Error updating Party OCPI version.')
        );

        // OCPI GET call for Versions Details of the Party, store OCPI endpoints.
        Log::channel('ocpi')->info('['.$party->code.'] OCPI GET call for Versions Details of the Party for version '.$party->version);
        ...
        throw_if(
            ...
            new Exception('['.$party->code.'] Empty or invalid response for Versions Details.')
        );
        throw_if(
            $versionDetails['version'] !== $party->version,
            new Exception('['.$party->code.'] Version mismatch for Versions Details: requested '.$party->version.' / received '.$versionDetails['version'].'.')
        );

        // Set Party OCPI endpoints for version.
        Log::channel('ocpi')->info('['.$party->code.'] Set OCPI endpoints for version '.$party->version);
        $party->endpoints = collect($versionDetails['endpoints'])
            ->pluck('url', 'identifier')
            ->toArray();
        throw_if(
            ! Arr::has($party->endpoints, 'credentials'),
            new Exception('['.$party->code.'] Missing required `credentials` Module endpoint.')
        );

        throw_if(
            ! $party->save(),
            new Exception('['.$party->code.'] Error updating Party OCPI endpoints.')
        );
